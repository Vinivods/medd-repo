...
menu = st.sidebar.selectbox(
    ...
    ("Bem-vindo", "Login", "Validades", "Estoque", "Faltas", "Sair")
)
    ...
    with logar:
        ...
        if login_btn:
            if login == '' or senha_logar == '':
                ...
                try:
                    conn = pyodbc.connect("Driver={ODBC Driver 17 for SQL Server};Server=35.188.130.222;Database=Meddivina;UID=sqlserver;PWD=045031;")
                    cursor = conn.cursor()
                        ...
                        if se_cadastro_existe is not None:
                            ...
                            if senha_login == senha_logar:
                                ...
                                try:
                                    ...
                                    cursor.execute(f'SELECT COUNT(v.Medicamento) AS Qtt, c.Farmacia FROM Validades v JOIN Cadastro c ON v.LoginFarma = c.LoginFarma WHERE v.Data_vencimento <= DATEADD(DAY, 30, GETDATE()) AND v.LoginFarma = ? GROUP BY c.Farmacia', login)
                                    ...
                                    cursor.execute('SELECT cf.Medicamento, COUNT(cf.Medicamento) AS QttFaltas, cf.Tasy FROM ControleFaltas cf JOIN Cadastro c ON cf.Farmacia = c.Farmacia WHERE cf.Data_falta <= DATEADD(DAY, 30, GETDATE()) AND cf.LoginFarma = ? GROUP BY cf.Medicamento, cf.Tasy, c.Farmacia ORDER BY QttFaltas DESC', login)
    ...
    with st.container():
        with pesquisar_tab:
            ...
            if botao_pesquisar:
                ...
                result_proxy = cursor.execute(f"SELECT Tasy, Medicamento, Lote, CONVERT(varchar, Data_vencimento, 103) AS DataFormatada, Quantidade FROM Validades WHERE LoginFarma = ? AND Data_vencimento <= DATEADD(DAY, {intervalo_validades_pesquisar}, GETDATE()) ORDER BY Data_vencimento ASC", login_validade_pesquisar)
                ...
                if resultados:
                    ...
                    query = f"SELECT Medicamento, CONVERT(varchar, Data_vencimento, 103) AS Data_vencimento, Lote, Tasy, Quantidade FROM Validades WHERE Data_vencimento <= DATEADD(DAY, {intervalo_validades_pesquisar}, GETDATE()) ORDER BY Data_vencimento ASC"
                    print(query)
...
# *ESTOQUE
    ...
    with st.container():
        ...
        dias_antepe_round = round(dias_antepe, 2)
        dias_penul_round = round(dias_penul, 2)
        dias_ulti_round = round(dias_ulti, 2)
        ...
        with st.expander('Que cálculos foram usados?'):
            st.markdown('**Variação**: para encontrar a variação (%) da quantidade de dispensações de um mês para o outro, calculamos a (**mês atual** - **mês anterior**) / **mês anterior** * 100.')
    ...
    with pesquisar_falta_tab:
        ...
        if pesquisar_falta:
            if farmacia_falta_pesquisar == '' or login_farma_faltas == '':
                ...
                cursor.execute(f'SELECT Tasy, Medicamento, Quantidade, CONVERT(varchar, Data_falta, 103), Farmacia, LoginFarma FROM ControleFaltas WHERE LoginFarma = ? AND Farmacia = ? AND Data_falta >= DATEADD(DAY, -{intervalo}, GETDATE())', login_farma_faltas, farmacia_falta_pesquisar)
