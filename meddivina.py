...
if menu == "Bem-vindo":
    ...
    with st.container():
        ...
        if botao_adicionar:
            if email == '' or tasy == '' or medicamento == '' or lote == '' or data_vencimento == '' or tasy == 0:
    ...
    with st.container():
        ...
        if botao_pesquisar:
            ...
            if resultado:
                ...
                st.markdown(f"O medicamento **{medicamento_retorno} ({tasy_retorno})** com o lote **{lote_retorno}** irá vencer no dia **{data_retorno}**.")

            else:
                # st.write("Nenhuma informação para este lote.")
                st.toast("Nenhuma informação para este lote")
